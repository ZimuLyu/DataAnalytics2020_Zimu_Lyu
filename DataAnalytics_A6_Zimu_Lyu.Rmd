...
```{r}
train$delq <- as.numeric(as.character(train$delq))
test$delq <- as.numeric(as.character(test$delq))
set.seed(233)
logit <- glm(delq ~ ., data = train)
# summary(logit)
```
...
train2$delq <- as.integer(train2$delq)
vif(glm(delq ~ ., data = train2))
```

```{r, results='hide'}
logit_reduced <- step(logit) # too slow
```
```{r}
...
```{r}
set.seed(233)
train$delq <- as.factor(train$delq)
test$delq <- as.factor(test$delq)
...
```{r}
loans_test$delq <- as.numeric(as.character(loans_test$delq))
...
```{r}
loans_test$delq <- as.factor(loans_test$delq)
